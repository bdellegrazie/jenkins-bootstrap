...
function dtrack_team_get_uuid_of() {
  local -r _api="${1}"
  local -r _team="${2}"
  echo "${_api}" | jq -r --arg name "${_team}" '.[] | select(.name == $name) | .uuid'
}

function dtrack_team_get_first_api_key() {
  local -r _api="${1}"
  local -r _team="${2}"
  echo "${_api}" | jq -r --arg name "${_team}" '.[] | select(.name == $name) .apiKeys | map(.key) | first'
}

function dtrack_team_oidc_mapping_get_all() {
  local -r _api="${1}"
  local -r _team="${2}"
  echo "${_api}" | jq -r --arg name "${_team}" '.[] | select(.name == $name) | .mappedOidcGroups'
}
...
function dtrack_team_permissions_get_all() {
  local -r _api="${1}"
  local -r _team="${2}"
  echo "${_api}" | jq -r --arg name "${_team}" '.[] | select(.name == $name) | .permissions | map(.name)'
}
...
function dtrack_oidc_group_get() {
  local -r _api="${1}"
  local -r _name="${2}"
  echo "${_api}" | jq -r --arg name "${_name}" '.[] | select(.name == $name)'
}

function dtrack_oidc_group_get_uuid_of() {
  local -r _api="${1}"
  local -r _name="${2}"
  echo "${_api}" | jq -r --arg name "${_name}" '.[] | select(.name == $name) | .uuid'
}
...
if ! dtrack_oidc_group_exists_by_name "${_oidc_groups}" "Administrators" ; then
  echo "Creating OIDC Group \"Administrators\""
  dtrack_oidc_group_create "Adminstrators"
  _oidc_groups=$(dtrack_oidc_group_get_all)
fi
...
declare automation_team_permissions=$(dtrack_team_permissions_get_all "${_teams}" "Automation")
